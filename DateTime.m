...
%   Copyright 2014-2017 The MathWorks, Inc.
    ...
    properties(GetAccess='public', Dependent=true)
        ...
        %   the Preferences dialog box, see <a href="matlab:helpview(fullfile(docroot,'matlab/matlab_env/command-window-preferences.html'))">Set Command Window Preferences</a>. Datetime
    ...
    properties(GetAccess='private', Constant)
        ...
        noConstructorParamsSupplied = struct('ConvertFrom',false, 'InputFormat',false, 'Format',false, 'TimeZone',false, 'Locale',false, 'PivotYear',false, 'Epoch',false);
    end
    ...
    methods(Access = 'public')
        ...
        %   If INFMT does not include a time portion, DATETIME assumes midnight.
        %
        %   D = DATETIME(DS,'InputFormat',INFMT,'Locale',LOCALE) specifies the locale that
        %   DATETIME uses to interpret the strings or character vectors in DS. LOCALE must be
        %   a  character vector in the form xx_YY, where xx is a lowercase ISO 639-1 two-letter 
        %   language code and YY is an uppercase ISO 3166-1 alpha-2 country code, for example 
        %   ja_JP. LOCALE can also be the character vector 'system' to use the system locale
        %   setting.  All strings or character vectors in DS must have the same locale.
        %
        ...
        %   contains the two-digit year specifier 'yy'. The default is YEAR(DATETIME('NOW')) - 50.
        ...
        %
        %   Unless 'TimeZone' is specified, D is an unzoned datetime array.  For 'posixtime',
        %   'juliandate', and 'modifiedjuliandate', the values in D when it is unzoned
        %   represent datetimes in UTC (and not your local time zone) that correspond to the
        %   given values of X.
        %
        ...
        %   the epoch time. EPOCH is a scalar DATETIME or a date/time character vector,
        %   representing the epoch time.
        %
        %   D = DATETIME(...,'Format',FMT) creates D with the specified display format. FMT
        %   is a datetime format character vector. See the description of the <a href="matlab:doc('datetime.Format')">Format property</a>
        %   for details. The factory setting for the default is locale-dependent. For information
        %   on setting the default in the Preferences dialog, see <a href="matlab:helpview(fullfile(docroot,'matlab/matlab_env/command-window-preferences.html'))">Set Command Window Preferences</a>.  
        %   When creating D from strings or character vectors, specify FMT as 'preserveinput'
        %   to use the the 'InputFormat' parameter (or the format that was determined automatically
        %   if 'InputFormat' was not given). Specify FMT as 'default' to use the default
        %   display format.
        %
        %   D = DATETIME(...,'TimeZone',TZ,...) specifies the time zone used to interpret the
        %   input data, and that the datetimes in D are in. TZ is the name of a time zone region,
        %   e.g. 'America/Los_Angeles', or an ISO 8601 character vector of the form +HH:MM or
        %   -HH:MM. Specify 'UTC' to create datetimes in Universal Coordinated Time. Specify the
        %   empty character vector '' to create "unzoned" datetimes that do not belong to a specific
        %   time zone. Specify 'local' to create datetimes in the system time zone. See the
        %   description of the <a href="matlab:doc('datetime.TimeZone')">TimeZone property</a> for more details. When the input data are strings
        %   or character vectors that include time zone offsets such as 'EST' or 'CEST', DATETIME
        %   converts all datetimes to the time zone specified by TZ.
        %
        %   Examples:
        %
        ...
        %   See also DATETIME, DURATION
            ...
            elseif matlab.internal.datatypes.isCharStrings(inData)
                haveStrings = true;
            elseif isstring(inData) && isscalar(inData) % Named instants only work as scalar, similar to char
                [haveNamedInstant,yesterdayTodayTomorrow] = ismember(lower(inData),{'now' 'yesterday' 'today' 'tomorrow'});
                if ~haveNamedInstant
                    haveStrings = true;
                end
            ...
            if isempty(varargin)
                ...
                [convertFrom,inputFmt,fmt,tz,locale,pivot,epoch,supplied] = parseArgs(pnames, dflts, varargin{:});
        ...
        function p = posixtime(this)
            ...
            %   is UTC, and not your local time zone. P is a double array. To compute POSIX times
            %   with POSIXTIME, it is recommended that first you specify a time zone for T.
            %
...
try %#ok<ALIGN>
    ...
    if nargin == 0 || isempty(tz)
        % clock returns the time according to the system time zone setting. But if
        % there's a client value set to override the system setting, shift the
        % system clock's result as if the system was set to the client time zone.
        [localTZ,isClientOverride] = datetime.getsetLocalTimeZone();
        if isClientOverride
            systemTZ = datetime.getsetLocalTimeZone('system');
            thisData = timeZoneAdjustment(thisData,localTZ,'');
            thisData = timeZoneAdjustment(thisData,'',systemTZ,isdst);
        end
        tz = '';
    else
...
ds = struct('ERA', 1, ...
    ...
    'DAY_OF_YEAR', 13, ...
    'DAY_OF_WEEK', 14, ...
    'DAY_OF_WEEK_IN_MONTH', 15, ...
    'AM_PM', 16, ...
    'HOUR', 17, ...
    'HOUR_OF_DAY', 18, ...
    'MINUTE', 19, ...
    'SECOND', 20, ...
    'MILLISECOND_OF_DAY', 21, ...
    'ZONE_OFFSET', 22, ...
    'DST_OFFSET', 23);
end
